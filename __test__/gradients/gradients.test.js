...
const html = fs.readFileSync(path.resolve(__dirname, './gradients.html'), 'utf8');
const classesWithExpectedStyles = require('./gradients.classes');
  ...
  classesWithExpectedStyles.forEach((_class) => {
    _class.expectedCSS.forEach((expectedStyleObj) => {
      it(`[${_class.className}] adds ${expectedStyleObj[0]} with value of ${expectedStyleObj[1]}`, () => {
        console.log(getElementComputedStylesByClassName(_class.className)[expectedStyleObj[0]]);
        expect(
